...
public final class TopologicalSort {
     ...
     */
    public static <T> List<T> sort(final DirectedGraph<T> g) {
        /* Construct the reverse graph from the input graph. */
        final DirectedGraph<T> gRev = reverseGraph(g);
     ...
     */
    private static <T> void explore(final T node, final DirectedGraph<T> g,
                                    final List<T> ordering, final Set<T> visited,
        ...
        for(final T predecessor : g.edgesFrom(node)) {
     ...
     */
    private static <T> DirectedGraph<T> reverseGraph(final DirectedGraph<T> g) {
